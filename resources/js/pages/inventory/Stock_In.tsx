...
const initialStockIns: StockInRow[] = Array.from({ length: 37 }).map((_, idx) => {
  const item = itemsMaster[idx % itemsMaster.length];
  const day = (idx % 27) + 1;
  return {
    id: idx + 1,
    date: new Date(2025, 6, day, 10, 0, 0).toISOString(), // July 2025
    itemId: item.id,
    itemName: item.name,
    quantity: Math.floor(Math.random() * 50) + 1,
    supplier: ['PT Sumber Rejeki', 'CV Mitra Abadi', 'UD Maju Jaya'][idx % 3],
...
export default function Stock_In() {
  const [rows, setRows] = useState<StockInRow[]>(initialStockIns);

  // table states
  const [query, setQuery] = useState('');
  const [sortBy, setSortBy] = useState<'date' | 'itemName' | 'quantity' | 'supplier' | 'reference'>('date');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('desc');
  const [page, setPage] = useState(1);

  // 🔥 NEW: Date range (react-day-picker)
  ...
  const handleDelete = (id: number) => { if (confirm('Hapus data stock-in ini?')) setRows(prev => prev.filter(r => r.id !== id)); };
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `stock-in_${today}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };

  // Label display untuk tombol date-range
  const rangeLabel = (() => {
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-4">
          ...
          <div className="flex-1">
            <div className="relative w-full max-w-md">
              ...
              <input
                type="text"
                placeholder="Cari tanggal / item / supplier / ref / catatan..."
          ...
          {/* 🔥 Date Range Picker (Open Calendar) */}
                ...
                paginated.map((row) => (
                  <tr key={row.id} className="border-b last:border-b-0">
                    ...
                    <td className="px-4 py-2 flex gap-2">
                      {/* tooltips dibiarkan seperti sebelumnya */}
                      <TooltipProvider>
                        <Tooltip>
        ...
        {sorted.length > 0 && (
          <div className="flex justify-between items-center mt-6 flex-wrap gap-2">
            <div className="text-sm text-muted-foreground">
              Total qty (filtered): <b>{totalQty}</b>
            </div>
