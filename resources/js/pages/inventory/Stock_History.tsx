...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: 'History Transactions', href: '/inventory' },
];
...
export default function Stock_History() {
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `stock-history_${formatDateISO(new Date())}.csv`;
    a.click();
      ...
      <div className="relative min-h-[100vh] flex-1 overflow-hidden rounded-xl border border-sidebar-border/70 md:min-h-min dark:border-sidebar-border bg-white dark:bg-background p-4">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-4">
          <form className="flex-1" onSubmit={handleSearchSubmit}>
            <div className="relative w-full max-w-md">
              ...
              <input
                type="text"
                placeholder="Cari item / type / party / ref / catatan..."
        ...
        <div className="flex justify-between items-center mt-6 flex-wrap gap-2">
          <div className="text-sm text-muted-foreground space-x-4">
            ...
            <span>NET: <b className={stats.net >= 0 ? 'text-emerald-700' : 'text-rose-700'}>{stats.net}</b></span>
            {meta.total > 0 && (
              <span className="ml-2">· {meta.total} data</span>
            )}
          </div>
          {meta.last_page > 1 && (
