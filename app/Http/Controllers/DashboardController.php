...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $costThisMonth = (int) SaleItem::whereHas('saleHeader', fn ($q) => $q->where('status', 'completed')
            ->whereBetween('occurred_at', [$monthStart, $monthEnd])
        ...
        $topItems = SaleItem::whereHas('saleHeader', fn ($q) => $q->where('status', 'completed')
            ->whereBetween('occurred_at', [$monthStart, $monthEnd])
            ->when(! empty($allowedIds), fn ($q2) => $q2->whereIn('warehouse_id', $allowedIds))
        ...
        $topProducts = SaleItem::whereHas('saleHeader', fn ($q) => $q->where('status', 'completed')
        ...
        if (empty($allowedIds)) {
            ...
            // 2 aggregate queries replace 3 × N per-warehouse queries
            $todayAgg = SaleHeader::where('status', 'completed')
                ->whereBetween('occurred_at', [$todayStart, $todayEnd])
                ->whereIn('warehouse_id', $warehouseIds)
                ->selectRaw('warehouse_id, SUM(grand_total) as total, COUNT(*) as trx_count')
                ->groupBy('warehouse_id')
                ->get()
                ->keyBy('warehouse_id');

            $monthAgg = SaleHeader::where('status', 'completed')
                ->whereBetween('occurred_at', [$monthStart, $monthEnd])
                ->whereIn('warehouse_id', $warehouseIds)
                ->selectRaw('warehouse_id, SUM(grand_total) as total')
                ->groupBy('warehouse_id')
                ->get()
                ->keyBy('warehouse_id');
            ...
            $branchStats = $warehouses->map(fn ($w) => [
                ...
                'salesMonth' => (int) ($monthAgg->get($w->id)?->total ?? 0),
                'trxToday' => (int) ($todayAgg->get($w->id)?->trx_count ?? 0),
            ])->all();
        }
